...
;to make-selection-choose [agent]
;  ;; el agente hace una eleccion con su par de acuerdo a la confianza
;  let possible-neighbor nobody
;  let i 0
;  let trust false
;  let lim-exec 0
;  print (word "agente " who " seleccionado con trial-capacity de: " trial-capacity)
;  while [(i < trial-capacity) and not trust]
;  [
;    set possible-neighbor one-of people
;    if agent != possible-neighbor and not member? possible-neighbor ([link-neighbors] of agent) [
;      set trust (confianza? agent possible-neighbor)
;      set i (i + 1)
;      print (word "aleatoriamente establecio contacto con el agente " ([ who ] of possible-neighbor)) 
;    ]
;    set lim-exec (lim-exec + 1)
;    if lim-exec > 100 [ set i (i + 1) ]  ;si se demora mucho en encontrar una persona, aumentar i
;  ]
;  ifelse trust [
;    ;; haz el link de confianza
;    ask agent [ create-link-with possible-neighbor ]
;    print (word "se creo un vinculo con el agente " ([ who ] of possible-neighbor) "porque hay confianza")
;    set num-trusted-links (num-trusted-links + 1)
;  ]
;  [
;    ;; de acuerdo a un esfuerzo de integración se decide si establecer una arista entre ambos agentes o no.
;    let aleatorio random-float 1.0
;    if aleatorio < 0.2 [ ;integration-endeavor
;      ;ask agent [ create-link-with one-of (people with [ who != ([who] of agent)])]
;      if possible-neighbor != agent [
;        ask agent [ create-link-with possible-neighbor ]
;        print (word "se creo un vinculo con el agente " ([ who ] of possible-neighbor) "aunque no habia confianza")
;        set num-untrusted-links (num-untrusted-links + 1)
;      ]
;    ]
;  ]
;end
...
to make-integration [ agent ]
  ...
  if (count vecinos) > 0 [
    ...
    ;;si el conj de conocidos-indirectos es vacio, quiere decir tu vecino seleccionado no tiene vecinos o todos sus contactos hacen parte de los del agente
  ...
  [
    ...
    if (count grupo-azar) > 0 [
      ...
      ;set selected-one one-of grupo-azar
      ...
      [
        ;;espacio para ver si se mete una probabilidad de traduccion o vatacion
...
;to-report confianza? [selection possible-neighbor]
;  ;; Devuelve true si possible-neighbor es mas confiable dado que ya está relacionado con alguno de sus vecinos.
;  let neighbours nobody
;  let salida false
;  ask selection [
;    set neighbours (turtle-set neighbours link-neighbors)
;  ]
;  if any? neighbours with [ link-neighbor? possible-neighbor ]
;  [
;    set salida true
;  ]
;  report salida
...
to save-exec [ejecucion]
  graph6:save-graph6 people links (word graph-file-location "integration-network-" num-people "-" stopTime "-" ejecucion ".g6")
  print (word "Graph was successfully saved on directory: " graph-file-location "integration-network-" num-people "-" stopTime "-" ejecucion ".g6")
  ;print timer
end

;; Experimento que toma muestras cronologicas de una red. Genera redes en ticks stopTime, un numero samples de veces.
to experiment
  setup
  ...
  repeat samples [
    repeat interval [ go ]
    save
    set stopTime (i * interval)
    set i (i + 1)
  ]
  set stopTime interval
end

;; procedimiento para tomar varias muestras de la red bajo una misma configuración de tiempo de parada
to take-samples
  let samples num-samples
  let i 1
  repeat samples [
...
;; Procedimiento para sacar varias muestras variando el tiempo, se debe primero crear las carpetas con la configuración que se quiera para que todo cuadre
;; Seleccionar en la interfaz la ruta del directorio donde estan las carpetas de cada muestras a tomar
;; initpartdirstring es el prefijo de la carpeta, numcarpetas es la cantidad de carpetas de las muestras, initialvalue es el valor en ticks que se inicia y
;; ticksinterval es el espacio o intervalo de tiempo. entre muestras
to granmuestreo [ initpartdirstring numcarpetas initialvalue ticksinterval ]
  let base graph-file-location
  let dir ""
  let ticcks initialvalue
  set stopTime ticcks
  
  repeat numcarpetas [
    set dir (word base initpartdirstring ticcks "\\")
    set graph-file-location dir
    
    take-samples
    
    set ticcks (ticcks + ticksinterval)
    set stopTime ticcks
  ]
...
;; procedimiento para tomar varias muestras de la red bajo una misma configuración de cantidad de links aproximados
to take-sampless
  let samples num-samples
  let i 1
  ...
  repeat samples [
    ...
    while [flag] [
      generate
      if (num-pioneer-links + num-trust-links + num-deliberation-links) >= nb-of-links [ set flag false ]
    ]
    save-exec i
    set i (i + 1)
  ]
...
;;se corrio asi doalgo [1 2 3 4 5] [300 400 500]
to doalgo [listlimcap listnumpeople]
  let base graph-file-location
  let dir ""
  let i 0
  ...
  foreach listlimcap [
    let j 0
    set dir (word base (item i listlimcap) "\\")
    ;print (word "estamos en carpeta lim-capacity: " item i lista "/")
    foreach listnumpeople [
      let k 0.1
      let p 10
      let direccion (word dir (item j listnumpeople) "\\")
      ;print (word "carpeta num-people: " item j otra "/")
      repeat 5 [
        
        let dirr (word direccion p "porcientocompletitud" "\\")
        set graph-file-location dirr
        ;print dirr                         ;;;Aqui va todo lo que se va a hacer
        
        set num-people (item j listnumpeople)
        let maxconnections ((num-people * (num-people - 1)) / 2)
        let enlaces (round (maxconnections * k))
        set limit-capacity (item i listlimcap)
        set nb-of-links enlaces
        set stopTime enlaces
        take-sampless
        ;print (word dirr "netwoe" enlaces)
...
La integración de los agentes para la emergencia de la coordinación se plantea mediante la confianza mutua que debe existir para crear un vinculo entre agentes. Confianza que se genera si existe almenos un amigo en común para los dos, de lo contrario se busca un candidato segun la cantidad de intentos (trials) que se especifiquen, si en ninguno de los intentos hay confianza para enlazarse; entonces, dada una probabilidad o esfuerzo de integración establecida los agentes deciden si se relacionan con el ultimo candidato de la lista o no.

## COMO UTILIZARLO

Inicialmente seleccione la cantidad de personas con las que piensa simular la red ajustando la barra **num-people**.

Ajuste la cantidad máxima de intentos o pruebas de busqueda que un agente puede hacer para encontrar un compañero confiable en la entrada con el nombre **trials**

Utilice el deslizador **integration-endeavor** para ajustar que tan dispuestos están los agentes a interconectarse con agentes no confiables.

Ingrese el tiempo en ticks que la simulación debe durar en la entrada **stopTime**.

Luego de configurar y ajustar los pasos anteriores, puedes presionar el botón **setup** para inicializar la configuración y seguido oprimir el botón **go** para correr la simulación.

Si desea descargar la red generada por el modelo luego de la simulación, puede hacerlo presionando el botón **save-network-graphml** no sin antes haber seleccionado un directorio o locación donde será guardado el archivo dando click en **select file location**.
