...
to make-integration [ agent ]
  ...
  let vecinos [link-neighbors] of agent                                         ;; Conjunto de todos los agentes con los cuales agent tiene enlaces
  let selected-one nobody                                                       ;; Declarando el agente que sera seleccionado para crear enlace
  let grupo-azar people with [ not member? self (turtle-set agent vecinos) ]    ;; Conjunto de todos los agentes sin relacion con agent
  ...
  ifelse (count vecinos) > 0 [
    ...
    ;;si el conjunto de conocidos-indirectos no es vacio, quiere decir que el vecino seleccionado tiene vecinos desconocidos para agente
    ifelse (count conocidos-indirectos) > 0 [
      ;;seleccionar al azar uno de los conocidos indirectos para crear un enlace de confianza mutua
    ...
    [
      ;;si el conjunto de conocidos-indirectos es vacio, quiere decir que el vecino seleccionado no tiene vecinos aún o todos sus contactos son compartidos con el agente
      ;; si el conjunto grupo-azar no es vacio, seleccionar un
      if (count grupo-azar) > 0 [
        let top-traductores sort-on [(- traduction-capacity)] grupo-azar
        ;; Seleccionar de la lista de top-traductores un agente al azar de los mejores calificados en traduction-capacity
        set selected-one item (round ((count grupo-azar) * 0.05)) top-traductores
        ;; LLamar a deliberación y votación a los vecinos 
        if (deliberation vecinos selected-one) [
          ;; Si la votacion de la deliberacion es exitosa, entonces crear enlace entre el agente y selected-one
          ask agent [ create-link-with selected-one ]
          ask selected-one [
            set color green
...
482
total links
num-deliberation-links + num-trust-links + num-pioneer-links
17
1
11
...
((num-deliberation-links + num-pioneer-links + num-trust-links) * 100) / ((num-people * (num-people - 1)) / 2)
2
1
11
